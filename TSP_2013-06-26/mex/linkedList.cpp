...
template <typename T>
void linkedListNode<T>::applyFunction(T (*func)(T))
{
   data = func(data);
}
template <typename T>
void linkedListNode<T>::applyFunction(T (*func)(T,T), T data2)
{
   data = func(data, data2);
}
...
linkedList<T>::linkedList(const linkedList& that) :
   first(NULL), last(NULL), length(0)
{
   linkedListNode<T>* node = that.getFirst();
   while (node != NULL)
   {
      addNodeEnd(node->getData());
      node = node->getNext();
   }
}
...
// --------------------------------------------------------------------------
template <typename T>
T linkedList<T>::dataAt(int index)
{
   linkedListNode<T>* node = first;
   int i=0;
...
template <typename T>
void linkedList<T>::append(linkedList<T> &b)
{
   if (b.length>0)
   {
      if (length>0)
