...
void OpticalFlow::JCLaplacian(DImage &output, const DImage &input, const DImage& weight, const DImage &deriv_mask)
{
   ...
   for(int i=0;i<height;i++)
      for(int j=0;j<width-1;j++)
      {
         ...
         if (fabs(deriv_mask[offset+1]-deriv_mask[offset])<1e-6 && deriv_mask[offset]>=-1e-6 && deriv_mask[offset+1]>=-1e-6)
   ...
   for(int i=0;i<height-1;i++)
      ...
      {
         int offset=i*width+j;
         if (fabs(deriv_mask[offset+width]-deriv_mask[offset])<1e-6 && deriv_mask[offset]>=-1e-6 && deriv_mask[offset+width]>=-1e-6)
...
// 
//--------------------------------------------------------------------------------------------------------
void OpticalFlow::JCSmoothFlowSOR(const DImage &Im1, const DImage &Im2, DImage &warpIm2, DImage &u, DImage &v, 
                      double alpha, int nOuterFPIterations, int nInnerFPIterations, int nSORIterations,
                      const DImage &deriv_mask, const DImage &bad_mask)
{
   DImage mask,imdx,imdy,imdt;
   int imWidth,imHeight,nChannels,nPixels;
   imWidth=Im1.width();
   ...
   for(int count=0;count<nOuterFPIterations;count++)
   {
      ...
      for(int hh=0;hh<nInnerFPIterations;hh++)
      {
         ...
         //================================================================================
         for (int w=0; w<imWidth; w++) for (int h=0; h<imHeight; h++)
         {
            int index = w + h*imWidth;
            if (w<imWidth-1 && fabs(deriv_mask[index]-deriv_mask[index+1])>1e-6 && deriv_mask[index]>=-1e-6 && deriv_mask[index+1]>=-1e-6)
            {
               ux[index] = 0;
               vx[index] = 0;
            }
            if (h<imHeight-1 && fabs(deriv_mask[index]-deriv_mask[index+imWidth])>1e-6 && deriv_mask[index]>=-1e-6 && deriv_mask[index+imWidth]>=-1e-6)
            {
               uy[index] = 0;
               vy[index] = 0;
            }
         ...
         // laplacian filtering of the current flow field
         JCLaplacian(foo1,u,Phi_1st,deriv_mask);
         JCLaplacian(foo2,v,Phi_1st,deriv_mask);
         ...
         for(int k = 0; k<nSORIterations; k++)
            for(int i = 0; i<imHeight; i++)
               for(int j = 0; j<imWidth; j++)
               {
                  ...
                  if(j>0 && fabs(deriv_mask[offset]-deriv_mask[offset-1])<1e-6 && deriv_mask[offset]>=-1e-6 && deriv_mask[offset-1]>=-1e-6)
                  {
                            _weight = phiData[offset-1];
                     sigma1  += _weight*du.data()[offset-1];
                     sigma2  += _weight*dv.data()[offset-1];
                  ...
                  if(j<imWidth-1 && fabs(deriv_mask[offset]-deriv_mask[offset+1])<1e-6 && deriv_mask[offset]>=-1e-6 && deriv_mask[offset+1]>=-1e-6)
                  {
                            _weight = phiData[offset];
                     sigma1 += _weight*du.data()[offset+1];
                     sigma2 += _weight*dv.data()[offset+1];
                  ...
                  if(i>0 && fabs(deriv_mask[offset]-deriv_mask[offset-imWidth])<1e-6 && deriv_mask[offset]>=-1e-6 && deriv_mask[offset-imWidth]>=-1e-6)
                  {
                            _weight = phiData[offset-imWidth];
                     sigma1 += _weight*du.data()[offset-imWidth];
                     sigma2 += _weight*dv.data()[offset-imWidth];
                  ...
                  if(i<imHeight-1 && fabs(deriv_mask[offset]-deriv_mask[offset+imWidth])<1e-6 && deriv_mask[offset]>=-1e-6 && deriv_mask[offset+imWidth]>=-1e-6)
                  {
                            _weight = phiData[offset];
                     sigma1  += _weight*du.data()[offset+imWidth];
                     sigma2  += _weight*dv.data()[offset+imWidth];
                  ...
                  if (bad_mask[offset] > 0 && false)
                  {
                     // compute du
                     du.data()[offset] = (1-omega)*du.data()[offset] + omega/(alpha*0.05 + coeff)*( - sigma1);
                     // compute dv
                     dv.data()[offset] = (1-omega)*dv.data()[offset] + omega/(alpha*0.05 + coeff)*( - sigma2);
                  }
                  else
                  {
                     // compute du
...
//--------------------------------------------------------------------------------------
void OpticalFlow::JCCoarse2FineFlow(DImage &vx, DImage &vy, DImage &warpI2,const DImage &Im1, const DImage &Im2, double alpha, double ratio, int minWidth, 
                                                    int nOuterFPIterations, int nInnerFPIterations, int nCGIterations, const DImage &deriv_mask, const DImage &bad_mask)
{
   // first build the pyramid of the two images
   GaussianPyramid GPyramid1;
   GaussianPyramid GPyramid2;
   GaussianPyramid GPyramid_deriv_mask;
   GaussianPyramid GPyramid_bad_mask;
   if(IsDisplay)
      cout<<"Constructing pyramid...";
   GPyramid1.ConstructPyramid(Im1,ratio,minWidth);
   GPyramid2.ConstructPyramid(Im2,ratio,minWidth);
   GPyramid_deriv_mask.ConstructPyramid(deriv_mask,ratio,minWidth);
   GPyramid_bad_mask.ConstructPyramid(bad_mask,ratio,minWidth);
   if(IsDisplay)
      cout<<"done!"<<endl;
   
   // now iterate from the top level to the bottom
   DImage Image1,Image2,Image_deriv_mask,Image_bad_mask,WarpImage2;
...
//      double aratio = width / GPyramid1.Image(0).width();
//      mexPrintf("width=%d  ratio=%0.2f\n", width, ratio);
      double aratio = 0.5;
      for (int i=0; i<width*height; i++)
      {
         double value = GPyramid_deriv_mask.Image(k)[i];
         if (value - (int)value < aratio || (int)value+1-value < aratio)
            GPyramid_deriv_mask.Image(k)[i] = double((int)(value+1e-6));
         else
            GPyramid_deriv_mask.Image(k)[i] = -1;

         value = GPyramid_bad_mask.Image(k)[i];
         if (value < aratio)
            GPyramid_bad_mask.Image(k)[i] = -1;
         else
            GPyramid_bad_mask.Image(k)[i] = 1;
      }

      /*mexPrintf("Deriv\n");
      imagesc(GPyramid_deriv_mask.Image(k).data(), width, height);
      waitforbuttonpress();
      mexPrintf("Bad\n");
      imagesc(GPyramid_bad_mask.Image(k).data(), width, height);
      waitforbuttonpress();*/
      ...
      JCSmoothFlowSOR(Image1,Image2,WarpImage2,vx,vy,alpha,nOuterFPIterations+k,nInnerFPIterations,nCGIterations+k*3, GPyramid_deriv_mask.Image(k), GPyramid_bad_mask.Image(k));
