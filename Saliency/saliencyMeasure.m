...
config.windowRows=[0.31]; % Window row size relative to image size max(imRow,imCol))
config.windowCols=[0.12]; % Window column size relative to image size max(imRow,imCol))
config.sampleStep=[0.04]; % Window sampling step relative to image size max(imRow,imCol)) (one for each window size)     
...
rgb = cat(3,dataCell{1},dataCell{2},dataCell{3});
% rgb = lab2rgb(rgb);
% rgb = im2uint8(rgb);
...
for winSizeId=1:length(config.windowRows)
    ...
    %Window confidence
    wc = zeros(1,numWindow);
    whist = zeros(numWindow,60,3);
    ...
    for winRowPos=rowCent
        for winColPos=colCent
            ...
            wh = w22-w11;
            window = rgb;
            [tmp,window] = draw_rect(window,[w11(2),w11(1)],[wh(2),wh(1)],0);
            wh = k22-k11;
            [tmp,window] = draw_rect(window,[k11(2),k11(1)],[wh(2),wh(1)],0);
            name = sprintf('window_%d.jpg',cnt-1);
            imwrite(window,name);
    ...
    for i=1:numWindow
        ...
        for j=1:numWindow
            if j~=i
                for c=1:3;
                    dist = pdist2(whist(i,:,c),whist(j,:,c),'emd');
                    if minDist > dist
                        minDist = dist;
                    end
    ...
    [maxV,maxId] = max(wc(:));
    fprintf('%dth window has the maximum confidence ',maxId)
